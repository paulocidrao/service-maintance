  ...
  <UContainer>
    <ClientOnly>
      <section class="mt-4 items-center justify-center flex flex-col">
        <UForm :schema="schema" :state="state" :validate-on="['input']" class="space-y-4 shadow bg-card p-4 rounded"
          @submit="onSubmit">
          ...
          <UFormField required label="Nome do cliente" name="clientName">
            <UInput v-model="state.clientName" autofocus highlight type="text" color="neutral"
              placeholder="Digite o nome do cliente" class="w-full" :ui="{ base: 'placeholder:text-white' }" />
          </UFormField>
          <UFormField required label="Email do cliente" name="userEmail">
            <UInput v-model="state.userEmail" highlight type="text" color="neutral"
              placeholder="Digite o email do cliente" class="w-full" :ui="{ base: 'placeholder:text-white' }" />
          </UFormField>
          ...
          <UFormField label="Orçamento" name="price">
            <UInput v-model="state.price" highlight type="number" color="neutral" placeholder="Digite um valor estimado"
              class="w-full" :ui="{ base: 'placeholder:text-white' }" />
          </UFormField>
          ...
          <UButton type="submit" class="cursor-pointer w-full justify-center" inactive-class="font-bold"
            color="neutral">
            Cadastrar serviço
          </UButton>
        </UForm>
